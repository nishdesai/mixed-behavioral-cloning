...
class NnValueFunction(object):
    ...
    def fit(self, X, y):
        ...
        if self.should_preproc:
            model.add(Dense(32, input_shape=(2*self.ob_dim,), activation='relu'))
        else:
            model.add(Dense(32, input_shape=(self.ob_dim,), activation='relu'))
        model.add(Dense(32, input_shape=(32,), activation='relu'))
        model.add(Dense(1, input_shape=(32,)))

        model.compile(optimizer='adam', loss='mean_squared_error', lr=self.learning_rate)
        if self.should_preproc:
            model.fit(self.preproc(X), y, nb_epoch=self.n_epochs)
        else:
            model.fit(X, y, nb_epoch=self.n_epochs)
        self.model = model
...
def main_cartpole(n_iter=100, gamma=1.0, min_timesteps_per_batch=1000, stepsize=1e-2, animate=False, logdir=None, seed=0, vf_type='linear', vf_params={}):
...
def main_pendulum(logdir, seed, n_iter, gamma, min_timesteps_per_batch, initial_stepsize, desired_kl, vf_type, vf_params, animate=False):
    ...
    means_na = dense(sy_h2, ac_dim, 'means', weight_init=normc_initializer(0.1)) # Mean control output
    logstd_a = tf.get_variable("logstdev", [ac_dim], initializer=tf.zeros_initializer(), dtype=tf.float32) # Variance

    sy_n = tf.shape(sy_ob_no)[0]
    # sampled actions, used for defining the policy (NOT computing the policy gradient)
    new_normal = tf.contrib.distributions.Normal(mu=means_na, sigma=tf.exp(logstd_a))
    # sy_sampled_ac = tf.clip_by_value(new_normal.sample(), env.action_space.low, env.action_space.high)
    sy_sampled_ac = new_normal.sample()
    sy_sampled_ac = tf.reshape(sy_sampled_ac, [-1,])

    # log-prob of actions taken -- used for policy gradient calculation
    sy_logprob_n = tf.diag_part(new_normal.log_pdf(sy_ac_n))

    # The following quantities are just used for computing KL and entropy, JUST FOR DIAGNOSTIC PURPOSES >>>>
    sy_oldmeans_na = tf.placeholder(shape=[None, ac_dim], name='oldmeans', dtype=tf.float32) # means BEFORE update (just used for KL diagnostic)
    sy_oldlogstd_a = tf.placeholder(shape=[ac_dim], name='oldlogstd', dtype=tf.float32) # old sd BEFORE update (just used for KL diagnostic)
    old_normal = tf.contrib.distributions.Normal(mu=sy_oldmeans_na, sigma=tf.exp(sy_oldlogstd_a))
    sy_kl = tf.reduce_mean(tf.contrib.distributions.kl(new_normal, old_normal))
    sy_ent = tf.reduce_mean(new_normal.entropy()) 
    # <<<<<<<<<<<<<

    sy_surr = - tf.reduce_mean(sy_adv_n * sy_logprob_n) # Loss function that we'll differentiate to get the policy gradient ("surr" is for "surrogate loss")

    sy_stepsize = tf.placeholder(shape=[], dtype=tf.float32) # Symbolic, in case you want to change the stepsize during optimization.
    optimizer = tf.train.AdamOptimizer(sy_stepsize)
    update_op = optimizer.minimize(sy_surr)
    ...
    for i in range(n_iter):
        ...
        # Policy update
        _, old_means, old_logstd = sess.run([update_op, means_na, logstd_a], feed_dict={sy_ob_no:ob_no, sy_ac_n:ac_n, sy_adv_n:standardized_adv_n, sy_stepsize:stepsize})
        kl, ent = sess.run([sy_kl, sy_ent], feed_dict={sy_ob_no:ob_no, sy_oldmeans_na:old_means, sy_oldlogstd_a:old_logstd})
...
if __name__ == "__main__":
    if 1:
        params = [
            dict(logdir='/tmp/ref1/linearvf-kl2e-3-seed0', seed=0, vf_type='linear', vf_params={}),
            dict(logdir='/tmp/ref1/nnvf-kl2e-3-seed0', seed=0, vf_type='nn', vf_params=dict(n_epochs=10, stepsize=1e-3)),
            dict(logdir='/tmp/ref1/linearvf-kl2e-3-seed1', seed=1, vf_type='linear', vf_params={}),
            dict(logdir='/tmp/ref1/nnvf-kl2e-3-seed1', seed=1, vf_type='nn', vf_params=dict(n_epochs=10, stepsize=1e-3)),
            dict(logdir='/tmp/ref1/linearvf-kl2e-3-seed2', seed=2, vf_type='linear', vf_params={}),
    ...
    if 0:
        ...
        params = [
            dict(logdir='/tmp/ref1/linearvf-kl2e-3-seed0', seed=0, desired_kl=2e-3, vf_type='linear', vf_params={}, **general_params),
            dict(logdir='/tmp/ref1/nnvf-kl2e-3-seed0', seed=0, desired_kl=2e-3, vf_type='nn', vf_params=dict(n_epochs=10, stepsize=1e-3), **general_params),
            dict(logdir='/tmp/ref1/linearvf-kl2e-3-seed1', seed=1, desired_kl=2e-3, vf_type='linear', vf_params={}, **general_params),
            dict(logdir='/tmp/ref1/nnvf-kl2e-3-seed1', seed=1, desired_kl=2e-3, vf_type='nn', vf_params=dict(n_epochs=10, stepsize=1e-3), **general_params),
            dict(logdir='/tmp/ref1/linearvf-kl2e-3-seed2', seed=2, desired_kl=2e-3, vf_type='linear', vf_params={}, **general_params),
            dict(logdir='/tmp/ref1/nnvf-kl2e-3-seed2', seed=2, desired_kl=2e-3, vf_type='nn', vf_params=dict(n_epochs=10, stepsize=1e-3), **general_params),
        ]
        import multiprocessing
        p = multiprocessing.Pool()
